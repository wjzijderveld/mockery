...
class CallStoreDirector
{
    ...
    /**
     * Checks if the given arguments match a call store. If not, it throws
     * an exception that no call was made with these arguments
     *
     * @param mixed
     * @return \Mockery\CallStore
     */
    public function with()
    {
        $args = func_get_args();
        $store = $this->findExpectation($args);
        if (is_null($store)) {
            throw new \Mockery\Exception(
                'Call to ' . get_class($this->_mock) . ' with '
                . \Mockery::formatArgs($this->_name, $args) . ' was not received'
                . ' as asserted'
            );
        }
        return $store;
